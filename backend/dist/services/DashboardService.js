...
class DashboardService {
    async getDashboardStats() {
        const [activeAttackers, totalEvents, stolenCredentials, compromisedHosts, avgDwellTime, blockedAttacks] = await Promise.all([
    ...
    async getMitreMatrix(attackerId) {
        ...
        events.forEach(event => {
            if (matrix[event.tactic]) {
                const existing = matrix[event.tactic].techniques.find((t) => t.id === event.technique);
                if (!existing) {
        ...
        Object.keys(matrix).forEach(tactic => {
            ...
            if (techniqueCount === 0)
                matrix[tactic].color = 'none';
            else if (techniqueCount <= 2)
                matrix[tactic].color = 'low';
            else if (techniqueCount <= 4)
                matrix[tactic].color = 'medium';
            else
                matrix[tactic].color = 'high';
        });
        return matrix;
    }
    ...
    async getCommandActivity(attackerId, limit = 10) {
        ...
        if (attackerId)
            query.attackerId = attackerId;
        const events = await models_1.AttackEvent.find(query).sort({ timestamp: -1 }).limit(limit).lean();
        return events.map(e => ({
    ...
    async getAttackerBehaviorAnalysis(attackerId) {
        ...
        const behaviors = {
            ...
            credentialDumping: events.filter(e => e.description.toLowerCase().includes('mimikatz') ||
                e.description.toLowerCase().includes('credential')).length > 0,
